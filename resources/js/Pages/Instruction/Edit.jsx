...
export default function Edit({ guide }) {
    const desc = guide.data_json?.description || '';
    const items = guide.data_json?.items || [];

    const { data, setData, post, processing, errors } = useForm({
        title: guide.kategori || '',
        description: desc,
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        post(`/instruction/${guide.id}/edit`);
    };

    return (
        <>
            <Head title="Edit Panduan Teknis - Web CME" />
            ...
            <div className="mb-6 flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-sm text-gray-500 font-headlines">
                        Perbarui rincian spesifikasi kriteria dan scope of work (SOW) lapangan.
                    </p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6 max-w-4xl">
                ...
                <div className="space-y-4">
                    ...
                    {data.items.map((item, itemIdx) => (
                        <Card
                            key={itemIdx}
                            title={item.parameter_name ? `Parameter: ${item.parameter_name}` : `Parameter #${itemIdx + 1}`}
                                            ...
                                            {item.sow.map((step, sIdx) => (
                                                <li key={sIdx} className="flex justify-between items-center text-xs text-gray-600 bg-gray-50 p-2 rounded border border-gray-200">
                                                    ...
                                                    <button
                                                        ...
                                                        className="text-red-500 hover:text-red-755 transition ml-2 font-bold"
                                                    >
                                                        &times;
                                                    </button>
                                                </li>
